...
export async function POST(request: Request, context: { params: Promise<{ projectId: string; assetId: string }> }) {
  try {
    ...
    const access = await createControlledAssetAccess({ projectId, assetId, actor: user, mode: "preview" });
    return Response.json({
      ok: true,
      data: {
        ...access,
        message: access.expiresInSeconds ? "已生成临时预览链接。系统已记录本次访问。" : "已记录外部资料预览操作。",
      },
    });
  } catch (error) {
