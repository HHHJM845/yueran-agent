...
- 甲方审核多版本档案必须保留当前基线：解锁任意当前审核入口后展示项目级审核档案，按 Brief、创意提案 Round 1、创意提案 Round 2、报价、合同、完整剧本、人物/场景设定、分镜图片、A copy、B copy 的业务顺序分组；同一 `(reviewType, targetScopeId)` 序列重新生成审核入口时，旧 `active` 版本必须置为 `expired`；档案区展示 `active/submitted/approved/rejected/expired/revoked`，排除 `draft`，且不得展示历史审核链接或验证码；文本节点（Brief、报价、合同、完整剧本）必须从已保存 item 快照渲染完整内容并支持当前版 vs 上一版/同序列任选两版并排对比；图片/视频节点（创意提案 Round 1/2、人物/场景设定、分镜图片、A copy、B copy）必须冻结生成审核版本当时的耐久素材引用 `ossUrl`/资源 id，不得把 1 小时签名 URL 当作历史快照保存，档案展开和版本并排对比必须通过受验证码保护的历史版本接口按需读取冻结 items 并在读取时重新签名展示 URL，不能回查当前项目素材覆盖历史版本；甲方审核页结论按钮和逐项结论必须使用 `通过` / `不通过`，状态文案必须统一为 `待审核/已提交/已通过/未通过/已过期/已撤销`。
