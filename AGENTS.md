...
- SOP 1 标准化 Brief 表格渲染层必须保留轻量强调标记支持，包括加粗、文字色和背景色；不得引入重型富文本框架替代当前轻量渲染。
...
- SOP 3 甲方通过 Round 1 后必须持久化保留方向；Round 2 只对保留方向继续深化，且必须按“扩展深化故事稿 -> 生成 700-800 字完整故事 -> 人工确认完整故事 -> 精选 4 个精彩场景 -> 生成深化视觉图”的顺序执行。这里的 4 个精彩场景只用于生成深化视觉图，不得恢复成“精彩分镜场景”或正式文字分镜拆解；正式文字分镜只属于 SOP 5。
